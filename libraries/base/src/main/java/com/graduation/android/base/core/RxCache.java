package com.graduation.android.base.core;
...
import com.graduation.android.base.model.CacheResult;
import com.graduation.android.base.utils.L;
import com.graduation.android.base.utils.ReflectUtil;
import com.graduation.android.base.utils.Utils;
...
//        this.cacheTime = builder.cacheTime;
     ...
     * @param type      缓存clazz
     */
//    public <T> ObservableTransformer<T, T> transformer(CacheMode cacheMode, final Type type) {
//        return transformer(cacheMode, null, -1L, type);
//    }
     ...
     * @param type      缓存clazz
     */
    public <T> ObservableTransformer<T, T> transformer(CacheMode cacheMode, final String key, final long time, final Type type) {
        L.e("rxcache", "RxCache transformer key=" + key);
            ...
            public ObservableSource<T> apply(@NonNull Observable<T> upstream) {
                ...
                if (type instanceof ParameterizedType) {//自定义ApiResult
                    ...
                    if (CacheResult.class.isAssignableFrom(cls)) {
                        tempType = ReflectUtil.getParameterizedType(type, 0);
                    }
                }
                return strategy.execute(RxCache.this, key, time, upstream, tempType).map(new Function<CacheResult<T>, T>() {
                    @Override
    ...
    public <T> Observable<T> load(final Type type, final String key, final long time) {
        return Observable.create(new BaseSimpleSubscribe<T>() {
            @Override
            T execute() {
    ...
    public <T> Observable<Boolean> save(final String key, final T value) {
        return Observable.create(new BaseSimpleSubscribe<Boolean>() {
            @Override
