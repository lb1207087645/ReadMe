...
import com.graduation.android.readme.base.adapter.listener.OnItemDragListener;
import com.graduation.android.readme.base.adapter.listener.OnItemSwipeListener;

import java.util.Collections;
import java.util.List;

/**
 * Created by linyt on 2016/7/13.
 *
 * @param <T> data type
    ...
    public void onBindViewHolder (K holder, int positions) {
        ...
        if (mItemTouchHelper != null && itemDragEnabled && viewType != LOADING_VIEW && viewType != HEADER_VIEW && viewType != EMPTY_VIEW && viewType != FOOTER_VIEW) {
            if (mToggleViewId != NO_TOGGLE_VIEW) {
                ...
                if (toggleView != null) {
                    toggleView.setTag(R.id.common_adapter_viewholder_support, holder);
                    if (mDragOnLongPress) {
                ...
                holder.itemView.setTag(R.id.common_adapter_viewholder_support, holder);
            ...
            mOnToggleViewLongClickListener = new View.OnLongClickListener() {
                ...
                public boolean onLongClick (View v) {
                    if (mItemTouchHelper != null && itemDragEnabled) {
                        mItemTouchHelper.startDrag((RecyclerView.ViewHolder) v.getTag(R.id.common_adapter_viewholder_support));
                    }
                    return true;
                }
            };
